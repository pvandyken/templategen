...
rule gen_init_avg_template:
    input: 
        lambda wcards: inputs[wcards['channel']].expand()
    output: tempout('iter_0/init/init_avg_template_{channel}.nii.gz')
    params:
        dim = config['ants']['dim'],
        use_n4 = '2',
        vox_dims=' '.join([str(d) for d in config['resample_vox_dims']])
    log: 'code/logs/gen_init_avg_template_{channel}.log'
...
rule get_existing_template:
    ...
    output: tempout('iter_0/init/existing_template_{channel}.nii.gz')
...
def choose_init_template(wcards):
    ...
    return rules.gen_init_avg_template.output[0].format(**wcards)

rule resample_init_template:
    input: choose_init_template
    output: tempout('iter_0/init/init_resampled_template_{channel}.nii.gz')
    log: 'code/logs/resample_init_avg_template_{channel}.log'
    container: config['singularity']['ants']
    params:
        dim = config['ants']['dim'],
        vox_dims=' '.join([str(d) for d in config['resample_vox_dims']])
...
rule set_init_template:
    ...
    output: tempout('iter_0/template_{channel}.nii.gz')
    group: 'init_template'
